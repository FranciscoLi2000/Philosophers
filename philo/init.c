...
*/
static intparse_args(t_rules *rules, int argc, char **argv)
{
longvalue; /* temporary storage for parsed numbers */
...
*/
static intinit_mutexes(t_rules *rules)
{
inti; /* loop index for mutex arrays */
...
*/
static voidinit_philos(t_rules *rules)
{
inti; /* loop index for philosophers */
...
rules->philos[i].left_fork = &rules->forks[i]; /* fork on the left */
rules->philos[i].right_fork = &rules->forks[(i + 1)
% rules->philo_count]; /* fork on the right */
i++; /* move to the next philosopher */
}
}
